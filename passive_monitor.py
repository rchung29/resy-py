"""Passive calendar monitor service.

Watches for availability changes (cancellations, unexpected drops) and
auto-books via the existing checkout flow.

Usage:
    python -m passive_monitor
"""
...
async def main() -> None:
    settings = Settings()
    setup_logger("passive-monitor", settings.log_level)

    all_restaurants = load_restaurants()
    restaurants = [r for r in all_restaurants if r.enabled and r.passive_monitor]

    if not restaurants:
        log.warning("no_restaurants_configured", msg="No restaurants have passive_monitor: true")
        return
    ...
    async def on_slots_discovered(msg: DiscoveredSlotsMessage) -> None:
        """Called by CalendarMonitor when slots are found."""
    ...
    for sig_ in (signal.SIGINT, signal.SIGTERM):
        loop.add_signal_handler(sig_, lambda: asyncio.create_task(shutdown(monitor)))

    log.info("passive_monitor_started")
    await monitor.run()


async def shutdown(monitor: CalendarMonitor) -> None:
